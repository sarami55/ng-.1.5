...
static int  s_put_s();
static int  s_put_c();
static VOID my_chsize();
static VOID my_chsize2();
  ...
  int  n;
{
  ...
  while (n-- > 0){
    if (_mb_gapend == _mb_size)
      break;
    my_chsize(&_mb_buf[_mb_gapend], &v, &m);
    _mb_gapend  += m;
    _mb_bufsize -= m;
  }
  return mb_point();
  ...
  int  n;
{
  ...
  while ((n-- > 0) && (_mb_point > _mb_prompt)){
    my_chsize2(&_mb_buf[_mb_point-1], &v, &m);
    mb_move(-m);
  }
  return mb_point();
}
  ...
  int n;
{
  ...
  if (n > 0){
	...
	if (mb_isbol()){
      ...
      my_chsize2(&_mb_buf[_mb_point-1], &v1, &m1);
      for (i = m1-1; i >= 0; i--)
	s[i] = _mb_buf[--_mb_point];
      my_chsize(&_mb_buf[_mb_gapend], &v2, &m2);
      for (i = 0; i < m2; i++)
  ...
  int n;
{
  ...
  if (n > 0){
    while (n > 0){
      my_chsize(&_mb_buf[_mb_point-1], &v, &m);
      while ((m--) > 0) 
	_mb_buf[_mb_point++] = _mb_buf[_mb_gapend++];
      --n;
    }
